...
// Progress callback.
//   step  : current progress (0..steps)
//   steps : total number of progress units for the current phase
//   time  : elapsed time per unit, in seconds
//   phase : human readable description of the current phase, e.g.
//           "loading model", "sampling", "decoding"
//   data  : user pointer registered with sd_set_progress_callback
// Returns true to request cancellation of the ongoing generation; the
// library stops at the earliest safe point and generate_image/generate_video
// returns NULL. Return false to continue.
typedef bool (*sd_progress_cb_t)(int step, int steps, float time, const char* phase, void* data);
