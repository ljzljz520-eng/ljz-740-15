...
// Latch set when a progress callback returns true (cancel request).
static std::atomic<bool> sd_progress_abort_flag{false};
// Phase associated with progress events that don't carry an explicit phase
// (e.g. tiled VAE processing). Set by the generation flow on this thread.
static thread_local const char* tl_progress_phase = "";

void sd_set_current_phase(const char* phase) {
    tl_progress_phase = phase ? phase : "";
}

bool sd_progress_aborted() {
    return sd_progress_abort_flag.load();
}

void sd_progress_reset_abort() {
    sd_progress_abort_flag.store(false);
}
...
bool pretty_progress(int step, int steps, float time, const char* phase) {
    if (sd_progress_cb) {
        bool abort_requested = sd_progress_cb(step, steps, time,
                                              phase ? phase : tl_progress_phase,
                                              sd_progress_cb_data);
        if (abort_requested) {
            sd_progress_abort_flag.store(true);
        }
...
void sd_set_progress_callback(sd_progress_cb_t cb, void* data) {
    ...
    // a new callback starts a fresh cancellation state
    sd_progress_abort_flag.store(false);
}
