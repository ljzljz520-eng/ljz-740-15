...
bool ModelLoader::load_tensors(on_new_tensor_cb_t on_new_tensor_cb, int n_threads_p, bool enable_mmap) {
    ...
    for (size_t file_index = 0; file_index < file_paths_.size(); file_index++) {
        ...
        for (int i = 0; i < n_threads; ++i) {
            workers.emplace_back([&, file_path, is_zip]() {
                ...
                while (true) {
                    ...
                    if (idx >= file_tensors.size() || failed || sd_progress_aborted()) {
                        break;
                    }
        ...
        while (true) {
            ...
            pretty_progress(static_cast<int>(curr_num), static_cast<int>(total_tensors_to_process), (ggml_time_ms() - t_start) / 1000.0f / (curr_num + 1e-6f), "loading model");
        ...
        pretty_progress(static_cast<int>(total_tensors_processed), static_cast<int>(total_tensors_to_process), (ggml_time_ms() - t_start) / 1000.0f / (total_tensors_processed + 1e-6f), "loading model");
