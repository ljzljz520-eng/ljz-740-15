...
bool suppress_pp(int step, int steps, float time, const char* phase, void* data) {
    (void)step;
    (void)steps;
    (void)time;
    (void)phase;
...
public:
    ...
    ggml_tensor* sample(ggml_context* work_ctx,
        ...
        auto denoise = [&](ggml_tensor* input, float sigma, int step) -> ggml_tensor* {
            if (sd_progress_aborted()) {
                LOG_INFO("sampling cancelled by progress callback");
                return nullptr;
            }
            ...
            auto sd_preview_mode    = sd_get_preview_mode();
            if (step == 1 || step == -1) {
                if (pretty_progress(0, (int)steps, 0, nullptr) || sd_progress_aborted()) {
                    LOG_INFO("sampling cancelled by progress callback");
                    return nullptr;
                }
            }
            ...
            if (step > 0 || step == -(int)steps) {
                ...
                if (pretty_progress(showstep, (int)steps, (t1 - t0) / 1000000.f / showstep, nullptr)) {
                    LOG_INFO("sampling cancelled by progress callback");
                    return nullptr;
                }
...
sd_image_t* generate_image_internal(sd_ctx_t* sd_ctx,
    ...
    pretty_progress(0, 0, 0, "encoding prompt");
    ...
    sd_set_current_phase("sampling");
    for (int b = 0; b < batch_count; b++) {
        if (sd_progress_aborted()) {
            LOG_INFO("generation cancelled by progress callback");
            break;
        }
        ...
        } else if (sd_progress_aborted()) {
            LOG_INFO("sampling cancelled after %.2fs", (sampling_end - sampling_start) * 1.0f / 1000);
            break;
        } else {
    ...
    sd_set_current_phase("decoding");
    pretty_progress(0, 0, 0, "decoding");
    std::vector<struct ggml_tensor*> decoded_images;  // collect decoded images
    for (size_t i = 0; i < final_latents.size(); i++) {
        if (sd_progress_aborted()) {
            LOG_INFO("decoding cancelled by progress callback");
            sd_set_current_phase("");
            ggml_free(work_ctx);
            return nullptr;
        }
        t1                      = ggml_time_ms();
...
sd_image_t* generate_image(sd_ctx_t* sd_ctx, const sd_img_gen_params_t* sd_img_gen_params) {
    // a new generation always starts from a non-cancelled state
    sd_progress_reset_abort();
    sd_set_current_phase("");
...
SD_API sd_image_t* generate_video(sd_ctx_t* sd_ctx, const sd_vid_gen_params_t* sd_vid_gen_params, int* num_frames_out) {
    ...
    if (high_noise_sample_steps > 0) {
        ...
        sd_set_current_phase("sampling (high noise)");
    ...
    sd_set_current_phase("");
    if (sd_progress_aborted() || vid == nullptr) {
        ggml_free(work_ctx);
        return nullptr;
    }
