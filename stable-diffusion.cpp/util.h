...
// Report progress for the given phase. Returns true when the registered
// progress callback requested cancellation.
bool pretty_progress(int step, int steps, float time, const char* phase = nullptr);
// Returns true after a progress callback has requested cancellation.
// Reset automatically at the beginning of each generate_image/generate_video.
bool sd_progress_aborted();
void sd_progress_reset_abort();
// Set the phase string attached to progress events without an explicit phase
// (notably tiled VAE decode progress and denoise step progress). Thread-local.
